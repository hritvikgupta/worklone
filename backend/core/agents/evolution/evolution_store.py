...
class EvolutionStore:
    ...
    def _init_db(self):
        ...
        conn.executescript("""
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS idx_els_employee_title ON employee_learned_skills(employee_id, title);
    ...
    def upsert_skill(self, employee_id: str, title: str, description: str, content: str) -> dict:
        conn = self._get_conn()
        try:
            ...
            conn.execute(
                ...
                INSERT INTO employee_learned_skills
                    (id, employee_id, title, description, content, version, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, 1, ?, ?)
                ON CONFLICT(employee_id, title) DO UPDATE SET
                    description = excluded.description,
                    content     = excluded.content,
                    version     = employee_learned_skills.version + 1,
                    updated_at  = excluded.updated_at
                """,
                (skill_id, employee_id, title, description, content, now, now),
            )
            row = conn.execute(
                "SELECT id, version, created_at FROM employee_learned_skills WHERE employee_id = ? AND title = ?",
                (employee_id, title),
            ).fetchone()
            conn.commit()
    ...
    def delete_skill(self, employee_id: str, title: str) -> bool:
        conn = self._get_conn()
        cur = conn.execute(
            "DELETE FROM employee_learned_skills WHERE employee_id = ? AND title = ?",
            (employee_id, title),
        )
        conn.commit()
        conn.close()
    ...
    def list_skills(self, employee_id: str) -> List[dict]:
        conn = self._get_conn()
        rows = conn.execute(
            "SELECT id, title, description, version, updated_at FROM employee_learned_skills WHERE employee_id = ? ORDER BY updated_at DESC",
            (employee_id,),
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]

    def list_skills_full(self, employee_id: str, limit: int = 10) -> List[dict]:
        ...
        rows = conn.execute(
            ...
            "WHERE employee_id = ? ORDER BY updated_at DESC LIMIT ?",
            (employee_id, limit),
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]

    def get_skill(self, employee_id: str, title: str) -> Optional[dict]:
        conn = self._get_conn()
        row = conn.execute(
            "SELECT * FROM employee_learned_skills WHERE employee_id = ? AND title = ?",
            (employee_id, title),
        ).fetchone()
        conn.close()
