...
class DiscordArchiveThreadTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {
            ...
            "required": ["threadId", "archived", "serverId"],
        }
    ...
    async def execute(self, parameters: dict, context: dict = None) -> ToolResult:
        ...
        url = f"https://discord.com/api/v10/channels/{parameters['threadId']}"
        body = {
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code in [200, 201, 204]:
                    try:
                        data = response.json()
                        message = "Thread archived successfully" if data.get("archived", False) else "Thread unarchived successfully"
