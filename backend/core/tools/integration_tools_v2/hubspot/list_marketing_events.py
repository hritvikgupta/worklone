...
class HubspotListMarketingEventsTool(BaseTool):
    ...
    async def execute(self, parameters: dict, context: dict = None) -> ToolResult:
        ...
        base_url = "https://api.hubapi.com/marketing/v3/marketing-events"
        query_params = []
        if parameters.get("limit"):
            query_params.append(f"limit={parameters['limit']}")
        if parameters.get("after"):
            query_params.append(f"after={parameters['after']}")
        query_string = "&".join(query_params)
        url = base_url if not query_string else f"{base_url}?{query_string}"
