...
class IncidentioIncidentTimestampsShowTool(BaseTool):
    name = "incidentio_incident_timestamps_show"
    ...
    async def _resolve_access_token(self, context: dict | None) -> str:
        connection = await resolve_oauth_connection(
            "incidentio",
            context=context,
            context_token_keys=("incidentio_api_key", "api_key"),
    ...
    async def execute(self, parameters: dict, context: dict = None) -> ToolResult:
        ...
        url = f"https://api.incident.io/v2/incident_timestamps/{parameters['id']}"
