...
class IncidentioCustomFieldsDeleteTool(BaseTool):
    ...
    async def _resolve_access_token(self, context: dict | None) -> str:
        return (context.get("INCIDENTIO_API_KEY") if context else "") or ""
    ...
    async def execute(self, parameters: dict, context: dict = None) -> ToolResult:
        ...
        url = f"https://api.incident.io/v2/custom_fields/{parameters['id']}"
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code in [200, 201, 204]:
                    return ToolResult(
                        success=True,
                        output="Custom field successfully deleted",
                        data={"message": "Custom field successfully deleted"},
                    )
                else:
