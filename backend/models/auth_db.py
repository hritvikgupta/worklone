...
class AuthDB:
    ...
    def create_chat_session(self, user_id: str, title: str = "New Chat", model: Optional[str] = None) -> str:
    ...
    def get_chat_session(self, session_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if user_id:
                row = conn.execute(
                    "SELECT id, user_id, title, model, created_at, updated_at FROM chat_sessions WHERE id = ? AND user_id = ?",
                    (session_id, user_id),
                ).fetchone()
            else:
                row = conn.execute(
                    "SELECT id, user_id, title, model, created_at, updated_at FROM chat_sessions WHERE id = ?",
                    (session_id,),
                ).fetchone()
            return dict(row) if row else None
        finally:
    ...
    def list_chat_sessions(self, user_id: str, limit: int = 100) -> list[Dict[str, Any]]:
