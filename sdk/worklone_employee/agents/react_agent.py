...
from worklone_employee.providers.config import get_provider_config, detect_provider, get_headers, get_payload_extras
from worklone_employee.db.store import EmployeeStore
from worklone_employee.types import EmployeeActivity, ActivityType, EmployeeStatus
from worklone_employee.tools.catalog import DEFAULT_EMPLOYEE_TOOL_NAMES, create_tool, expand_tool_selection
from worklone_employee.tools.registry import ToolRegistry
from worklone_employee.tools.employee.ask_user_tool import ASK_USER_MARKER
from worklone_employee.tools.employee.run_task_tool import RUN_TASK_MARKER
...
from worklone_employee.evolution.background_review import spawn_memory_review, spawn_skill_review
...
class GenericEmployeeAgent:
    ...
    def __init__(
        ...
        # Config passed directly — no DB reads for these
        name: str = "Employee",
        role: str = "Generalist",
        description: str = "",
        system_prompt: str = "",
        ...
        skills: Optional[List] = None,          # static EmployeeSkill objects
        extra_tool_names: Optional[List[str]] = None,  # catalog tool names
    ):
        ...
        # Config lives in memory — passed from Employee(), never read from DB
        from worklone_employee.types import Employee as EmployeeModel, EmployeeStatus
        ...
        self.employee_skills = skills or []
        self._extra_tool_names = extra_tool_names or []
        ...
        # Self-evolution — only DB reads in the whole agent
        ...
        self.model = model
        self.provider_name = llm_config.get("provider_name") or detect_provider(model)
    ...
    def _learned_skills_text(self) -> str:
        skills = self.evolution_store.list_skills_full(self.employee_id, self.user_id, limit=10)
        if not skills:
    ...
    def _register_tools(self):
        """Register default tools plus any extra catalog tools passed at init."""
        seen = set()
        ...
        for tool_name in expand_tool_selection(self._extra_tool_names):
            tool = create_tool(tool_name)
            if not tool:
    ...
    def _set_employee_status(self, status: EmployeeStatus) -> None:
        ...
        if not store:
            return
        try:
            store.update_employee(self.employee_id, {"status": status.value}, self.owner_id)
        except Exception as e:
    ...
    async def _run_background_task(self, task_id: str, instructions: str) -> None:
        ...
        from worklone_employee.types import TaskStatus, ActivityType, EmployeeActivity, EmployeeStatus
