...
async def _run_skill_review(
    ...
    if result and result.get("should_save") and result.get("title") and result.get("content"):
        ...
        logger.info("[evolution] Skill %s for employee=%s owner=%s: %s", info["action"], employee_id, owner_id, result["title"])
...
def spawn_skill_review(
    ...
    _REVIEW_POOL.submit(
        _thread_runner,
        lambda: _run_skill_review(employee_id, owner_id, employee_role, model, messages_snapshot, store),
    )
